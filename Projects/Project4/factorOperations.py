...
def joinFactors(factors):
    ...
    for factor in factors:
        for conditionedVar in factor.conditionedVariables():
            joinedConditionedVariables.add(conditionedVar)
            
        for unconditionedVar in factor.unconditionedVariables():
            joinedUnconditionedVariables.add(unconditionedVar)

    for unconditionedVar in joinedUnconditionedVariables:
        if unconditionedVar in joinedConditionedVariables:
            joinedConditionedVariables.remove(unconditionedVar)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for reducedAssignment in reducedFactor.getAllPossibleAssignmentDicts():
            ...
            reducedFactor.setProbability(reducedAssignment, probability)
...
def normalize(factor):
    ...
    initialProbabilities = 0.0

    for assignment in factor.getAllPossibleAssignmentDicts():
        initialProbabilities += factor.getProbability(assignment)
        
    for variable in factor.unconditionedVariables():
        if len(factor.variableDomainsDict()[variable]) == 1:
            oneEntVars.append(variable)
    
    for variable in factor.conditionedVariables():
        
        conditionedVars.add(variable)

    for variable in oneEntVars:
        conditionedVars.add(variable)

    for variable in factor.unconditionedVariables():
        if variable not in oneEntVars:
            unconditionedVars.add(variable)

    normal = Factor(unconditionedVars, conditionedVars, factor.variableDomainsDict())


    for assignment in normal.getAllPossibleAssignmentDicts():
        normalProb = factor.getProbability(assignment) / initialProbabilities
        normal.setProbability(assignment, normalProb)
